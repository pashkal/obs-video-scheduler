...
(function ($) {
    ...
    TimeSlider.prototype.init = function(element, options) {
        ...
        this.$element.find('.graduation-title-top').remove();
        this.$element.append(
        ...
        if (this.$element.attr('contest_timestamp')) {
            options['contest_timestamp'] = parseInt(this.$element.attr('contest_timestamp'));
        }
    ...
    TimeSlider.prototype.set_options = function (options) {
        if (options.hours_per_ruler) {
            ...
            if (options.hours_per_ruler != this.options.hours_per_ruler) {
                ...
                this.set_contest_caret_position();
    ...
    TimeSlider.prototype.add_contest_caret = function() {
        this.$ruler.append('<div class="contest-start-caret"></div>');
        this.contest_caret = this.$ruler.find('.contest-start-caret');
        this.set_contest_caret_position();
    };
    ...
    TimeSlider.prototype.add_graduations = function() {
        ...
        for (var i = -4; i <= num_steps; i++) {
            ...
            this.$ruler.append('<div id="top-hour' + i + '" class="graduation top' + caret_class + '" style="left: ' + nleft.toString() + 'px"></div>');
    ...
    TimeSlider.prototype.toggle_timecell = function(timecell) {
        ...
        if (this.running_time_cell) {
            ...
            this.$prompts.append(
                '<div id="r-prompt-' + timecell_id + '" class="prompt" style="top:101px;left: ' + (left + width - 44).toString() + 'px;width:10px;">' +
                    '<div class="triangle-up"></div>' +
                    '<div class="body">' + this.timestamp_to_time_from_start(timecell['start']) + '</div>' +
                '</div>');
    ...
    TimeSlider.prototype.add_timecell = function(timecell) {
        ...
        if (typeof this.options.on_add_timecell_callback == 'function') {
            this.options.on_add_timecell_callback(timecell_id, start, stop, name);
        }
    };
    ...
    TimeSlider.prototype.report_all_timecells = function(reportFunction) {
        var timecells = [];
        var _this = this;
    ...
    TimeSlider.prototype.remove_graduations = function() {
        ...
        this.$ruler.find('.graduation-title-top').remove();
    };
    ...
    TimeSlider.prototype.set_running_elements = function() {
        ...
        return function() {
            ...
            _this.set_contest_caret_position();
...
//              console.log(this.options.contest_timestamp);
    ...
    TimeSlider.prototype.set_time_cell_position = function(diff_x) {
        ...
        const old_start = parseInt(this.time_cell_selected.element.attr('start_timestamp'));
        const old_stop = parseInt(this.time_cell_selected.element.attr('stop_timestamp'));
        const duration = old_stop - old_start;
        var new_start = old_start + Math.round(this.current_diff_x / this.px_per_ms);

        var closest_sticky_time = old_start;
        for (var cell of this.all_cells) {
            // Not stick to itself's boundaries
            if (cell._id == id) {
                continue;
            }

            // Allow sticking to end of another video
            var sticky_time = cell.stop;
            if (Math.abs(sticky_time - new_start) < Math.abs(closest_sticky_time - new_start)) {
                closest_sticky_time = sticky_time;
            }

            // Allow sticking to beginning of another video
            sticky_time = cell.start - duration;
            if (Math.abs(sticky_time - new_start) < Math.abs(closest_sticky_time - new_start)) {
                closest_sticky_time = sticky_time;
            }
        }
        ...
        var rem = (new_start - this.options.contest_timestamp) % (1000 * 60);
        if (rem < 0) {
            rem += 1000 * 60;
        }

        // Stick to previous even minutes from start
        if (rem < Math.abs(closest_sticky_time - new_start)) {
            closest_sticky_time = new_start - rem;
        }
        // Stick to next even minutes from start
        if (1000 * 60 - rem < Math.abs(closest_sticky_time - new_start)) {
            closest_sticky_time = new_start - rem + 1000 * 60;
        }

        if (closest_sticky_time == old_start) {
            diff_x = 0;
        } else {
            diff_x = (closest_sticky_time - old_start) * this.px_per_ms;
            this.current_diff_x -= diff_x;
        }
